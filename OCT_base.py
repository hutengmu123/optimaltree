# -*- coding: utf-8 -*-
"""
Created on Sun Sep 25 13:03:39 2022
...
def get_warm_start(model, **kwargs):
    ...
    return model
# %%
def solve_oct(X_transformed, y_transformed, L_hat, epsilons,
...
def solve_oct_MILP(X_transformed, y_transformed, L_hat, epsilons,
                   ...
                   solver="gurobi", small_epsilon=False,epsilon_version=False,verbose=False, log_file=None, **kwargs):
    """Solve the Optimal Classification Tree by Mixed-Integer Linear Programming
    
    
    adding small_epsilon:
        if true, epsilon = 0.00000001
    adding epsilon_version:
        if true, epsilon in the parenthesis
 ...
 #   pair_branch_nodes_left_right_leaf_nodes = _get_pair_nodes_childs(temp)
    ...
    if small_epsilon == True:
        epsilon_min = small_number
    else:
        epsilon_min = np.min(epsilons)
    epsilon_max = np.max(epsilons)
...
#        epsilon_max = 1.5 * epsilon_min
    ...
    if epsilon_version == True:
        def split_left_cons(model, i, t, m):
        # i in model.I, t and m in model.tL_AL pair which represent (leaf node, left ancestor node)
        # left hand side
            lhs = sum(model.X[i, j]*model.a[j, m]
                  for j in model.J) + sum(model.a[j,m]*model.epsilons[j] for j in model.J)
...
#%%
def solve_modified_oct_MILP(X_transformed, y_transformed, L_hat, epsilons,
                   alpha=0.01, max_depth=2, min_samples_leaf=1,small_number = 0.0000001,
                   solver="gurobi", small_epsilon=False,epsilon_version=False,verbose=False, log_file=None, **kwargs):
    """Solve the Optimal Classification Tree by Mixed-Integer Linear Programming
